...
import { useInstitutionRating, useInstitutionReviews, useSubmitInstitutionReview } from '@/portal/hooks/useInstitutionReviews';
...
export function ProfileReviews({ slug }: Props) {
  const { data: ratingData } = useInstitutionRating(slug);
  const [page, setPage] = useState(0);
  const pageSize = 5;
  const { data: reviewsData } = useInstitutionReviews(slug, page, pageSize);
  const submitReview = useSubmitInstitutionReview();
  ...
  return (
    <div id="reviews" className="scroll-mt-20 space-y-6 mt-10">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          ...
          <span className="text-sm text-muted-foreground">{page + 1} з {totalPages}</span>
        ...
        <Card>
          <CardContent className="p-4 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={formText} onChange={e => setFormText(e.target.value)} placeholder="Опишіть ваш досвід..." className="min-h-[80px]" maxLength={1000} />
            </div>
            <div className="space-y-1.5">
