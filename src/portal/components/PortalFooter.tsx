...
const portalLinks = [
  { label: "Оподаткування", href: "/taxes" },
  { label: "ФОП", href: "/fop" },
  { label: "Фізособам", href: "/individuals" },
  { label: "Бухоблік", href: "/accounting" },
  { label: "Законодавство", href: "/law" },
  { label: "Бізнес і війна", href: "/wartime" },
];

const toolLinks = [
  { label: "AI-консультант", href: "/consultant" },
  { label: "AI-форум", href: "/forum" },
  { label: "Калькулятори", href: "/tools" },
  { label: "Радар змін", href: "/radar" },
  { label: "Довідники", href: "/dovidnyky" },
  { label: "Словник", href: "/dovidnyky/slovnyk" },
];

const companyLinks = [
  { label: "Огляд", href: "/overview" },
  { label: "Дайджест", href: "/newsletter" },
  { label: "Реклама на порталі", href: "/reklama" },
...
export const PortalFooter = () => (
  <footer className="border-t border-border/50 py-12">
    <nav aria-label="Footer" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 mb-8">
        ...
        <FooterColumn title="Портал" links={portalLinks} />
        <FooterColumn title="Інструменти" links={toolLinks} />
        <FooterColumn title="Компанія" links={companyLinks} />

        {/* Contacts */}
        <div className="space-y-3">
          <h4 className="font-semibold text-sm text-foreground mb-3">Контакти</h4>
          <div className="flex items-center gap-2 text-sm text-muted-foreground min-w-0">
            <Mail className="w-4 h-4 shrink-0" />
            <a href="mailto:support@fintodo.ua" className="hover:text-foreground hover:underline transition-colors truncate min-w-0">
              support@fintodo.ua
            </a>
          </div>
          <div className="flex items-center gap-2 text-sm text-muted-foreground min-w-0">
            <Phone className="w-4 h-4 shrink-0" />
            <a href="tel:+380441234567" className="hover:text-foreground hover:underline transition-colors truncate min-w-0 whitespace-nowrap">
              +38 (044) 123-45-67
            </a>
          </div>
