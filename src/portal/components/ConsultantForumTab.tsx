...
export const ConsultantForumTab = () => {
  const { items: aiConsultations, isLoading: forumLoading } = useMergedForumData();
  ...
  }, [debouncedSearch, activeTag, setSearchParams, searchParams]);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <div className="flex-1 min-w-0 space-y-5">
        ...
        <div className="sticky top-14 z-10 bg-background/95 backdrop-blur-sm -mx-4 sm:-mx-6 px-4 sm:px-6 py-2 border-b border-border/40 shadow-sm">
          ...
          <CollapsibleContent className="overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 duration-200">
            {allTags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pb-1 pt-1">
                {activeTag && (
                  <Badge variant="destructive" className="cursor-pointer text-xs" onClick={() => setActiveTag(null)}>
                    ✕ Скинути
                  </Badge>
                )}
                {allTags.map((tag) => (
      ...
      <aside className="hidden lg:block lg:w-72 xl:w-80 shrink-0 space-y-4 lg:sticky lg:top-20 lg:self-start">
        <div className="rounded-xl border border-border/60 bg-card overflow-hidden">
          ...
          <div className="p-4 space-y-3 text-xs text-muted-foreground">
            ...
            <Button onClick={() => setSearchParams({ tab: "chat" })} size="sm" className="w-full gap-2 mt-3">
