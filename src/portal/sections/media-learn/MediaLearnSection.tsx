...
import { Mic, Video, GraduationCap, Play, Clock, Users, ChevronRight, ArrowRight } from "lucide-react";
...
type MediaTab = "podcast" | "video" | "courses";

const TAB_CONFIG: Record<MediaTab, { label: string; icon: typeof Mic; allLink: string; allLabel: string }> = {
  podcast: { label: "Подкасти", icon: Mic, allLink: "/publications/podcasts", allLabel: "Всі подкасти" },
  video: { label: "Відео", icon: Video, allLink: "/publications/videos", allLabel: "Всі відео" },
  courses: { label: "Курси", icon: GraduationCap, allLink: "/learn", allLabel: "Всі курси" },
};
...
function FeaturedCard({ article, tab }: { article: Article; tab: "podcast" | "video" }) {
  const Icon = tab === "podcast" ? Mic : Video;
  return (
    <Link
...
function CompactCard({ article, tab }: { article: Article; tab: "podcast" | "video" }) {
  const Icon = tab === "podcast" ? Mic : Video;
  const actionLabel = tab === "podcast" ? "Слухати" : "Дивитися";
...
export function MediaLearnSection() {
      ...
      >
        ...
        <div className="text-center mb-5">
          ...
          <p className="text-muted-foreground text-sm sm:text-base">
            Слухайте, дивіться, навчайтесь — все в одному місці
          </p>
        </div>
...
function MediaContent({ tab }: { tab: "podcast" | "video" }) {
  const filtered = ARTICLES.filter((a) => a.mediaType === tab);
  const [featured, ...rest] = filtered;

  return (
    <>
      {featured && <FeaturedCard article={featured} tab={tab} />}
      {rest.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-4">
          {rest.slice(0, 3).map((article) => (
            <CompactCard key={article.id} article={article} tab={tab} />
          ))}
        </div>
      )}
    </>
