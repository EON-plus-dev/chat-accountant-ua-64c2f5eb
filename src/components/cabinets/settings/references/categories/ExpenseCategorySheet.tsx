...
import type { ExpenseCategory } from "@/config/categoriesConfig";
...
export function ExpenseCategorySheet({
  ...
  const [group, setGroup] = useState<"operational" | "administrative" | "financial" | "other">(
    category?.group ?? "operational"
  );
  const [isDeductible, setIsDeductible] = useState(category?.isDeductible ?? true);
  ...
  const resetForm = useCallback(() => {
    ...
    setGroup(category?.group ?? "operational");
    setIsDeductible(category?.isDeductible ?? true);
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent side="right" className="flex flex-col p-0 w-full sm:max-w-md">
        <SheetHeader className="px-6 pt-6 pb-2">
          <SheetTitle>
            {isEdit ? "Редагувати категорію витрат" : "Нова категорія витрат"}
          </SheetTitle>
          <SheetDescription>
            {isEdit
              ? "Змініть параметри категорії"
              : "Створіть нову категорію для класифікації витрат"}
          </SheetDescription>
        </SheetHeader>
          ...
          <div className="space-y-5 pb-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Наприклад: Програмне забезпечення"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select
                value={group}
                onValueChange={(v) =>
                  setGroup(v as "operational" | "administrative" | "financial" | "other")
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <Label>Враховується у витратах</Label>
                <p className="text-xs text-muted-foreground">
                  Чи зменшує оподатковуваний дохід
                </p>
              </div>
              <Switch
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Автоматично обирається для нових витрат
                </p>
              </div>
